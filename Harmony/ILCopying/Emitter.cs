...
namespace Harmony.ILCopying
{
	public static class Emitter
	{
		...
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + FormatArgument(label));
			il.MarkLabel(label);
		}

		public static void Emit(ILGenerator il, OpCode opcode)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode);
			il.Emit(opcode);
		}

		public static void Emit(ILGenerator il, OpCode opcode, LocalBuilder local)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(local));
			il.Emit(opcode, local);
		}

		public static void Emit(ILGenerator il, OpCode opcode, FieldInfo field)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(field));
			il.Emit(opcode, field);
		}

		public static void Emit(ILGenerator il, OpCode opcode, Label[] labels)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(labels));
			il.Emit(opcode, labels);
		}

		public static void Emit(ILGenerator il, OpCode opcode, Label label)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(label));
			il.Emit(opcode, label);
		}

		public static void Emit(ILGenerator il, OpCode opcode, string str)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + FormatArgument(str));
			il.Emit(opcode, str);
		}

		public static void Emit(ILGenerator il, OpCode opcode, float arg)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(arg));
			il.Emit(opcode, arg);
		}
		...
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(meth));
			il.Emit(opcode, meth);
		}
		...
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(signature));
			il.Emit(opcode, signature);
		}

		public static void Emit(ILGenerator il, OpCode opcode, ConstructorInfo con)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(con));
			il.Emit(opcode, con);
		}

		public static void Emit(ILGenerator il, OpCode opcode, Type cls)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + opcode + " " + FormatArgument(cls));
			il.Emit(opcode, cls);
		}
		...
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + "Call " + opcode + " " + methodInfo + " " + optionalParameterTypes);
		...
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + "Calli " + opcode + " " + unmanagedCallConv + " " + returnType + " " + parameterTypes);
		...
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + "Calli " + opcode + " " + callingConvention + " " + returnType + " " + parameterTypes + " " + optionalParameterTypes);
		...
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + "WriteLine " + FormatArgument(value));
			il.EmitWriteLine(value);
		}

		public static void EmitWriteLine(ILGenerator il, LocalBuilder localBuilder)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + "WriteLine " + FormatArgument(localBuilder));
			il.EmitWriteLine(localBuilder);
		}

		public static void EmitWriteLine(ILGenerator il, FieldInfo fld)
		{
			if (PatchProcessor.DEBUG) FileLog.Log(CodePos(il) + "WriteLine " + FormatArgument(fld));
